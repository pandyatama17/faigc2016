...
<div class="form-group">
 ...
 @foreach($member_types as $mt)
     ...
     <div class="radio" id="radio_{{$mt->id}}">
        <label>
           <input required class="radioInput flat-green " type="radio" name="member_type" id="optionsRadios_{{$mt->id}}" data-cost="{{$mt->cost}}" value="{{$mt->id}}"
               @if(Session::has('registype'))
           ...
           {{$mt->name}}
           @if($mt->cost != 0)
                 <span class="badge">IDR {{$mt->cost}},-</span><sup>*Early Bird Registration until July 1<sup>st</sup> 2016</sup>
           @endif
           <br>
